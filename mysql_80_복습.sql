...
/***************************************************
	...
	** 실행 결과 컬럼이 동일(컬럼명, 데이터타입)
***************************************************/
-- 영업부, 정보시스템 부서의 사원아이디, 사원명, 급여, 부서아이디 조회
-- union : 영업부서 사원들이 한번만 출력
select 	emp_id	
		, emp_name
		, salary
        , dept_id
...
-- 				...
-- 			);
-- 데이터타입 : 정수형(int, long, ...), 실수형(float, double), 문자형(char, varchar, longtext, ...), 이진데이터(longblob), 날짜형(date,datetime), ...  
-- char(고정형 문자형) : 크기가 메모리에 고정되는 형식	예) char(10) --> 3자리 입력 : 7자리 낭비                
-- varchar(가변형 문자형) : 실제 저장되는 데이터 크기에 따라 메모리가 변경되는 형식	예) varchar(10) --> 3자리 입력 : 메모리에 실제 3자리 공간만 생성
...
-- 형식 : create table [테이블명]
-- 		 as [SQL 정의]
-- employee 테이블을 복제하여 emp 테이블 생성 -> 복제할 때 제약사항은 복제되지 않음
create table emp2
as 
select * from employee;
select * from emp2;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    형식 > insert into [테이블명] ({컬럼리스트 ...})
		  values(데이터1, 데이터2, ...)
+++++++++++++++++++++++++++++++++++++++++++++++++*/
insert into emp2 (emp_id, ename, gender, hire_date, salary)
...
-- phone 컬럼 크기 변경 : char(13) --> char(10)
alter table emp
	modify column phone char(10) null; -- 저장된 데이터보다 크기가 작으면 에러 발생; 데이터 유실 위험 발생!! 크기를 크게 하는건 상관 없음!
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
select * from emp2;
set sql_safe_updates = 0;	-- 업데이트 모드 해제. 창을 새로 띄울 때마다 해줘야함
-- 홍길동의 급여를 6000으로 수정
update emp2
	set 	salary = 6000
    where	ename = '홍길동';
...
-- dep_id 컬럼에 primary key 제약 추가
alter table dept
	add constraint pk_dept_id primary key(dept_id);
desc dept;

-- 2018년도에 입사한 사원들만 복제(emp)
create table emp
as
select *
from employee
...
-- 홍길동 학생이 수강하는 과목을 조회 (서브쿼리, 오라클 inner join, ansi inner join)
select	sname
from	subject
...
alter table student 
	add column kor	decimal(10, 2)	null;
alter table student 
	add column eng	decimal(10, 2)	null;
alter table student 
	add column math	decimal(10, 2)	null;
...
-- 입사일 : 입사년도, 급여 : 3자리 구분
select	row_number() over(order by emp_id) as rno
		, emp_id
		, emp_name
...
alter table employee_stru
	modify column emp_id	char(5)	primary key;
...
insert into employee_stru(emp_name, eng_name, gender, hire_date, retire_date, dept_id, phone, email, salary)
	values('홍길동', 'hong', 'M', curdate(), null, 'SYS', '010-1234-1234', 'hong@exercise.com', 1000);
    
insert into employee_stru(emp_name, eng_name, gender, hire_date, retire_date, dept_id, phone, email, salary)
	values('이순신', 'captin', 'M', curdate(), null, 'HRD', '010-1234-5678', 'captin@exercise.com', 2000);
...
-- 정주고 사원 삭제
delete from emp where emp_name = '정주고';

...
select	d.dept_name 부서명
		, d.dept_id 부서아이디
		, format(sum(e.salary), 0) 총급여
		, format(avg(e.salary), 0) 평균급여
		, sum(v.duration) 휴가사용일수
...
select	sname
from	student
where	sid = (select sid from subjcet where sid = (select sid from professor where name = '김철수'));
